...
//Verficiar Existencia del ID
   if (verificar_existenica($id,$con) == 1) {
    echo 2; //el registro existe retornado 2 el cual sera evaluado en js
   }else{
     /*$sql="INSERT INTO tb_sensores (ID,Nombre,Ubicacion,FechaRegistro,Estado) VALUES ('$id','$nombre','$ubicacion','$fecha','$estado')";*/
     $sql = "UPDATE tb_sensores SET Nombre = '$nombre', Ubicacion = '$ubicacion', FechaRegistro = '$fecha', Estado = '$estado' WHERE ID = '$id'";
...
function verificar_existenica($idv,$conexion){
  $verif = "SELECT * FROM tb_sensores WHERE ID = '$idv' AND Nombre != 'NA'";
